...
struct amdvgpu_recyclable_host_blob {
   struct list_head list;
   struct amdvgpu_host_blob *hb;
   uint64_t ts;
};

struct amdvgpu_host_blob_recycle_bin {
   struct amdvgpu_host_blob_allocator b;
...
static
void amdvgpu_host_blob_recycle_bin_destroy(amdvgpu_device_handle dev, struct amdvgpu_host_blob_allocator *a) {
   struct amdvgpu_host_blob_recycle_bin *bin = (struct amdvgpu_host_blob_recycle_bin*)a;
   struct amdvgpu_recyclable_host_blob *entry, *tmp;
   LIST_FOR_EACH_ENTRY_SAFE(entry, tmp, &bin->list, list) {
      destroy_host_blob(dev, entry->hb);
   }
   simple_mtx_destroy(&bin->mtx);
   free(bin);
}
static
struct amdvgpu_host_blob *recycle_host_blob_get(amdvgpu_bo_handle bo,
                                                struct amdvgpu_host_blob_allocator *a,
                                                struct drm_virtgpu_resource_create_blob *args,
                                                struct amdgpu_ccmd_gem_new_req *req,
                                                bool *va_map_done)
{
   amdvgpu_device_handle dev = bo->dev;
   struct amdvgpu_host_blob_recycle_bin *bin = (struct amdvgpu_host_blob_recycle_bin*)a;
   struct amdvgpu_recyclable_host_blob *entry, *tmp;
   uint64_t now = os_time_get_nano();

   simple_mtx_lock(&bin->mtx);

   LIST_FOR_EACH_ENTRY_SAFE_REV(entry, tmp, &bin->list, list) {
      struct amdvgpu_host_blob *candidate = entry->hb;

      if (candidate->vm_flags == req->vm_flags &&
          candidate->phys_alignment == req->r.phys_alignment &&
          candidate->preferred_heap == req->r.preferred_heap &&
          candidate->flags == req->r.flags &&
          candidate->alloc_size >= args->size) {
         list_del(&entry->list);
         free(entry);

         simple_mtx_unlock(&bin->mtx);

         /* Reuse this host blob (transfer ownership) */
         return candidate;
      }

      if (now - entry->ts < 5 * ONE_SECOND_IN_NS)
         continue;

      /* This buffer expired, delete it. */
      list_del(&entry->list);
      a->next->free_host_blob(dev, a->next, entry->hb);
      free(entry);
   }

   simple_mtx_unlock(&bin->mtx);

   return a->next->alloc_host_blob(bo, a, args, req, va_map_done);
}

static void recycle_host_blob_put(amdvgpu_device_handle dev,
                                  struct amdvgpu_host_blob_allocator *a,
                                  struct amdvgpu_host_blob *hb)
{
   struct amdvgpu_host_blob_recycle_bin *bin = (struct amdvgpu_host_blob_recycle_bin*)a;
   struct amdvgpu_recyclable_host_blob *entry = calloc(1, sizeof(*entry));
   entry->ts = os_time_get_nano();
   entry->hb = hb;

   simple_mtx_lock(&bin->mtx);
   list_add(&entry->list, &bin->list);
   simple_mtx_unlock(&bin->mtx);
}

struct amdvgpu_host_blob_allocator * amdvgpu_host_blob_recycle_bin_allocator_create(void)
{
   struct amdvgpu_host_blob_recycle_bin *bin = calloc(1, sizeof(*bin));
   simple_mtx_init(&bin->mtx, mtx_plain);
   list_inithead(&bin->list);
   bin->b.destroy = amdvgpu_host_blob_recycle_bin_destroy;
   bin->b.alloc_host_blob = recycle_host_blob_get;
   bin->b.free_host_blob = recycle_host_blob_put;
   return &bin->b;
}

struct amdvgpu_large_host_blob {
   struct list_head list;
   struct amdvgpu_host_blob *hb;
   uint64_t used;
   int refcount;
...
struct amdvgpu_host_blob_suballocator {
   struct amdvgpu_host_blob_allocator b;
...
static
void amdvgpu_host_blob_suballocator_destroy(amdvgpu_device_handle dev, struct amdvgpu_host_blob_allocator *a) {
   struct amdvgpu_host_blob_suballocator *bin = (struct amdvgpu_host_blob_suballocator*)a;
   struct amdvgpu_large_host_blob *entry, *tmp;
   LIST_FOR_EACH_ENTRY_SAFE(entry, tmp, &bin->list, list) {
      destroy_host_blob(dev, entry->hb);
   }
   simple_mtx_destroy(&bin->mtx);
...
static bool can_be_suballocated(amdvgpu_device_handle dev, uint64_t size,
                                uint64_t flags, uint32_t preferred_heap)
{
   if (dev->min_alloc_size <= size)
      return false;

   if (!(preferred_heap & (AMDGPU_GEM_DOMAIN_VRAM | AMDGPU_GEM_DOMAIN_GTT)))
      return false;

   /* Requiring VM_ALWAYS_VALID avoid the need to deal with duplicate
    * resource ids in CHUNK_BO_HANDLES.
    */
   return flags & (1llu << 63);
}

static
struct amdvgpu_host_blob *suballocator_host_blob_get(amdvgpu_bo_handle bo,
                                                     struct amdvgpu_host_blob_allocator *a,
                                                     struct drm_virtgpu_resource_create_blob *args,
                                                     struct amdgpu_ccmd_gem_new_req *req,
                                                     bool *va_map_done)
{
   struct amdvgpu_host_blob_suballocator *bin = (struct amdvgpu_host_blob_suballocator*)a;
   struct amdvgpu_large_host_blob *entry, *tmp, *compatible = NULL;
   bool can_suballoc = can_be_suballocated(bo->dev, args->size, req->r.flags, req->r.preferred_heap);

   if (!can_suballoc)
      goto fail;

   simple_mtx_lock(&bin->mtx);
   uint64_t flags = req->r.flags & ~(1llu << 63);

   LIST_FOR_EACH_ENTRY_SAFE_REV(entry, tmp, &bin->list, list) {
      struct amdvgpu_host_blob *candidate = entry->hb;

      uint64_t offset_in_cand = align(entry->used, MAX2(4096, req->r.phys_alignment));

      if (candidate->vm_flags == req->vm_flags &&
          candidate->phys_alignment == req->r.phys_alignment &&
          candidate->preferred_heap == req->r.preferred_heap &&
          candidate->flags == flags) {
         if ((offset_in_cand + args->size) <= candidate->alloc_size) {
            bo->suballocation_offset = offset_in_cand;
            entry->used = offset_in_cand + args->size;

            uint64_t space_left = candidate->alloc_size - entry->used;

            if (space_left < 0x1000) {
               /* Remove from the candidates list but don't free
                * the entry. It'll be freed when it's refcount
                * reaches 0.
                */
               list_del(&entry->list);
            }
            p_atomic_inc(&entry->refcount);
            simple_mtx_unlock(&bin->mtx);

            assert(candidate->opaque == entry);

            return candidate;
         } else {
            /* Keep a single compatible entry in the list; we'll check
             * below if entry should be removed from the list.
             */
            compatible = entry;
            break;
         }
   ...
   /* We didn't find an existing large buffer to suballocate from, allocate a new one.
    * Bump the allocation size and let the allocation be handled by the next allocator.
    */
   req->r.alloc_size = bo->dev->min_alloc_size;

fail:
   struct amdvgpu_host_blob *hb = a->next->alloc_host_blob(bo, a->next, args, req, va_map_done);

   if (can_suballoc) {
      if (hb) {
         struct amdvgpu_large_host_blob *e = calloc(1, sizeof(*e));
         e->used = args->size;
         e->hb = hb;
         e->hb->opaque = e;
         p_atomic_set(&e->refcount, 1);

         if (compatible) {
            if (e->used < compatible->used) {
               list_del(&compatible->list);
               list_add(&e->list, &bin->list);
            }
         } else {
            list_add(&e->list, &bin->list);
         }
...
static void suballocator_host_blob_put(amdvgpu_device_handle dev,
                                       struct amdvgpu_host_blob_allocator *a,
                                       struct amdvgpu_host_blob *hb)
{
   struct amdvgpu_host_blob_suballocator *bin = (struct amdvgpu_host_blob_suballocator*)a;
   struct amdvgpu_large_host_blob *entry = hb->opaque;
   ...
   if (p_atomic_dec_zero(&entry->refcount)) {
      if (list_is_linked(&entry->list))
         list_del(&entry->list);
      free(entry);
      hb->opaque = NULL;
...
next:
   a->next->free_host_blob(dev, a->next, hb);
}

struct amdvgpu_host_blob_allocator * amdvgpu_host_blob_suballocate_allocator_create(void)
{
   struct amdvgpu_host_blob_suballocator *bin = calloc(1, sizeof(*bin));
   simple_mtx_init(&bin->mtx, mtx_plain);
   list_inithead(&bin->list);
   bin->b.destroy = amdvgpu_host_blob_suballocator_destroy;
   bin->b.alloc_host_blob = suballocator_host_blob_get;
   bin->b.free_host_blob = suballocator_host_blob_put;
   return &bin->b;
}
...
static
void amdvgpu_host_blob_default_destroy(amdvgpu_device_handle dev, struct amdvgpu_host_blob_allocator *a) {
   free(a);
}
static
struct amdvgpu_host_blob *default_allocator_get(amdvgpu_bo_handle bo,
                                                struct amdvgpu_host_blob_allocator *a,
                                                struct drm_virtgpu_resource_create_blob *args,
                                                struct amdgpu_ccmd_gem_new_req *req,
                                                bool *va_map_done)
...
static void default_allocator_put(amdvgpu_device_handle dev,
                                 struct amdvgpu_host_blob_allocator *a,
                                 struct amdvgpu_host_blob *hb)
{
   destroy_host_blob(dev, hb);
}

struct amdvgpu_host_blob_allocator * amdvgpu_host_blob_default_allocator_create(void)
{
   struct amdvgpu_host_blob_allocator *def = calloc(1, sizeof(*def));
   def->destroy = amdvgpu_host_blob_default_destroy;
   def->alloc_host_blob = default_allocator_get;
   def->free_host_blob = default_allocator_put;
   return def;
}
                                   ...
                                   bool *va_map_done)
{
   ...
   bo->host_blob = dev->allocator->alloc_host_blob(bo, dev->allocator,
                                                   &args, &req, va_map_done);
...
int amdvgpu_bo_free(struct amdvgpu_bo *bo) {
   ...
   if (refcnt == 0) {
      ...
      if (bo->imported) {
         ...
         bo->dev->allocator->free_host_blob(bo->dev, bo->dev->allocator, bo->host_blob);
      }
