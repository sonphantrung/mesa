...
{
   radeon_info info;
   info.gfx_level = ctx->program->gfx_level;
   info.family = ctx->program->family;
   info.cp_sdma_ge_use_system_memory_scope = ctx->program->gfx_level == GFX12;
   return ac_get_hw_cache_flags(&info, (gl_access_qualifier)access);
}
