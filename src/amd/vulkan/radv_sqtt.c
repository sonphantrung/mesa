...
static void
radv_write_sqtt_commands_to_cs(const struct radv_device *device, struct radeon_cmdbuf *cs, enum radv_queue_family qf,
                               uint32_t *reg_val, int n)
{
   const struct radv_physical_device *pdev = radv_device_physical(device);
   ...
   for (int i = 0; i < n; i += 2) {
      ...
      if (reg == 0) {
         ...
         if (reg > 1)
            i += reg - 1;
      } else if (reg < SI_CONFIG_REG_END) {
         radeon_check_space(device->ws, cs, 6);
         radeon_set_privileged_config_reg(cs, reg, value);
      } else if (reg < SI_SH_REG_END) {
         radeon_check_space(device->ws, cs, 2);
         radeon_set_sh_reg(cs, reg, value);
      } else if (reg < CIK_UCONFIG_REG_END) {
         radeon_check_space(device->ws, cs, 3);
         if (SI_UCONFIG_PERF_REG_OFFSET <= reg && reg < SI_UCONFIG_PERF_REG_END)
            radeon_set_perfctr_reg(gfx_level, qf, cs, reg, value);
         else
            radeon_set_uconfig_reg(cs, reg, value);
      } else {
         assert(false);
      }
   }
...
{
   const struct radv_physical_device *pdev = radv_device_physical(device);
   enum amd_ip_type type = radv_queue_family_to_ring(pdev, qf);
   struct util_dynarray arr;
   util_dynarray_init(&arr, NULL);

   if (ac_prepare_sqtt_start(&pdev->info, &device->sqtt, type, &arr))
      radv_write_sqtt_commands_to_cs(device, cs, qf, arr.data, arr.size / sizeof(uint32_t));
...
radv_emit_sqtt_stop(const struct radv_device *device, struct radeon_cmdbuf *cs, enum radv_queue_family qf)
{
   ...
   if (ac_prepare_sqtt_stop(&pdev->info, &device->sqtt, type, &arr))
      radv_write_sqtt_commands_to_cs(device, cs, qf, arr.data, arr.size / sizeof(uint32_t));
...
radv_sqtt_init_bo(struct radv_device *device)
{
   ...
   device->sqtt.bo_va = radv_buffer_get_va(bo);
...
radv_sqtt_init(struct radv_device *device)
{
   ...
   sqtt->options.instructions_timing = radv_is_instruction_timing_enabled();
