                        ...
                        bool copy_memcpy)
{
   ...
   const char *src = (const char *) info->pHostPointer + start_layer * dst_layer_stride;
   ...
   for (unsigned layer = 0; layer < layers; layer++,
      ...
      if (copy_memcpy) {
         ...
         fdl6_memcpy_linear_to_tiled(offset.x, offset.y,
                                     ...
                                     info->imageSubresource.mipLevel,
                                     src_pitch,
                                     device->physical_device->highest_bank_bit);
      }

      if (dst_image->bo->cached_non_coherent) {
         tu_sync_cache_bo(device, dst_image->bo,
                        ...
                        const VkCopyMemoryToImageInfoEXT *info)
{
   ...
   if (dst_image->lrz_height) {
      tu_disable_lrz_cpu(device, dst_image);
   }
                        ...
                        bool copy_memcpy)
{
   ...
   for (unsigned layer = 0; layer < layers; layer++,
      ...
      if (src_image->bo->cached_non_coherent) {
         tu_sync_cache_bo(device, src_image->bo,
