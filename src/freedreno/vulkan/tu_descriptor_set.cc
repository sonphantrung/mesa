   ...
   case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
      return A6XX_TEX_CONST_DWORDS * 4 * (1 +
         COND(dev->physical_device->info->a6xx.storage_16bit &&
              !dev->physical_device->info->a6xx.has_universal_16bit_storage_descriptor, 1) +
                             ...
                             const VkDescriptorAddressInfoEXT *buffer_info)
{
    ...
    * 16-bit and 32-bit access will of course only be functional when 16-bit
    * storage is supported. */
   assert(!info->a6xx.has_universal_16bit_storage_descriptor || info->a6xx.storage_16bit);
   ...
   assert(!info->a7xx.storage_8bit || (info->a6xx.storage_16bit &&
                                       info->a6xx.has_universal_16bit_storage_descriptor));

   unsigned num_descriptors = 1 +
      COND(info->a6xx.storage_16bit &&
           !info->a6xx.has_universal_16bit_storage_descriptor, 1) +
    ...
    * 16-bit descriptor cannot be used for 32-bit isam loads.
    */
   if (!info->a6xx.storage_16bit || !info->a6xx.has_universal_16bit_storage_descriptor) {
