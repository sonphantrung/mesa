...
static bool si_sqtt_init_bo(struct si_context *sctx)
{
   ...
   sctx->sqtt->bo_va = sctx->ws->buffer_get_virtual_address(sctx->sqtt->bo);

   return true;
}

static void
si_write_sqtt_commands_to_cs(struct si_context *sctx, struct radeon_cmdbuf *cs, uint32_t *reg_val, int n)
{
   if (n == 0)
      return;
   ...
   for (int i = 0; i < n; i += 2) {
      ...
      if (reg == 0) {
         ...
         radeon_set_privileged_config_reg(reg, value);
         ...
         if (SI_UCONFIG_PERF_REG_OFFSET <= reg && reg < SI_UCONFIG_PERF_REG_END) {
            radeon_set_uconfig_perfctr_reg_seq(reg, 1);
            radeon_emit(value);
         } else {
            radeon_set_uconfig_reg(reg, value);
         }
      } else {
         assert(false);
      }
                               ...
                               enum amd_ip_type ip_type)
{
   ...
   if (ac_prepare_sqtt_start(&sctx->screen->info, sctx->sqtt, ip_type, &arr))
      si_write_sqtt_commands_to_cs(sctx, cs, arr.data, arr.size / sizeof(uint32_t));
                              ...
                              enum amd_ip_type ip_type)
{
   ...
   if (ac_prepare_sqtt_stop(&sctx->screen->info, sctx->sqtt, ip_type, &arr))
      si_write_sqtt_commands_to_cs(sctx, cs, arr.data, arr.size / sizeof(uint32_t));
