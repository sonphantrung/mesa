...
## with "_".
<%def name="to_camel(snake_str)">
    <%
    result = ''.join(word.title() for word in snake_str.split('_'))
    return result if not result.isdigit() else '_' + result
    %>
</%def>


## "type" and "override" are keywords in Rust, we must rename them.
<%
for mthd in mthddict:
    if 'TYPE' in mthddict[mthd].field_name_start:
        mthddict[mthd].field_name_start['TYPE_'] = mthddict[mthd].field_name_start.pop('TYPE')
        mthddict[mthd].field_name_end['TYPE_'] = mthddict[mthd].field_name_end.pop('TYPE')
        mthddict[mthd].field_defs['TYPE_'] = mthddict[mthd].field_defs.pop('TYPE')
    if 'OVERRIDE' in mthddict[mthd].field_name_start:
        mthddict[mthd].field_name_start['OVERRIDE_'] = mthddict[mthd].field_name_start.pop('OVERRIDE')
        mthddict[mthd].field_name_end['OVERRIDE_'] = mthddict[mthd].field_name_end.pop('OVERRIDE')
        mthddict[mthd].field_defs['OVERRIDE_'] = mthddict[mthd].field_defs.pop('OVERRIDE')
%>
...
%if 'V' in mthd.field_rs_types:
    %if mthd.field_rs_types['V'] == to_camel(mthd_name) + "V":
#[repr(u16)]
#[derive(Copy, Clone, Debug, PartialEq)]
pub enum ${to_camel(mthd_name)}V {
    %for field_name, field_value in mthd.field_defs['V'].items():
    ${to_camel(field_name)} = ${field_value.lower()},
    %endfor
}
    %endif

## Usually that will be just 'v', but some commands take v and other fields
## (like SET_RT_LAYER)
#[derive(Copy, Clone, Debug, PartialEq)]
pub struct ${to_camel(mthd_name)} {
  %for field_name in mthddict[mthd_name].field_name_start:
    pub ${field_name.lower()}: ${mthd.field_rs_types[field_name]},
  %endfor
}

## Otherwise, we define a struct with the fields.
%else:
#[derive(Copy, Clone, Debug, PartialEq)]
pub struct ${to_camel(mthd_name)} {
  %for field_name in mthddict[mthd_name].field_name_start:
...
impl Mthd for ${to_camel(mthd_name)} {
    fn addr(&self) -> u16 {${mthd.addr.replace('(', '').replace(')', '')}}
    ...
    fn to_bits(&self) -> u32 {
        ...
        %for field_name in mthddict[mthd_name].field_name_start:
            <%
                ...
                field_width = field_end - field_start + 1
                field = field_name.lower() if mthd.field_rs_types[field_name] == "u32" else f"{field_name.lower()} as u32"
            %>
            %if field_width == 32:
        val |= self.${field};
            %else:
